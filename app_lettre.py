    ...
    with col2:
        if st.button("✨ Générer ma lettre", key="btn_l"):
            if not is_premium and st.session_state.generations_count >= 1: afficher_paywall()
                ...
                res = generer_texte_gemini(f"Rédige une lettre de motivation pour {nom}, {poste} chez {entreprise}. Compétences: {competences}")
...
with tab3:
    if not is_premium: st.warning("🔒 Espace réservé aux membres Premium.")
        ...
        if outil == "Relance":
            ...
            if st.button("🚀 Créer", key="b_ent"): st.text_area("Conseils :", generer_texte_gemini(f"Questions entretien pour {p}"))
        elif outil == "LinkedIn":
